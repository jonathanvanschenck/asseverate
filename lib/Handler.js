...
class Handler extends Function {
    ...
    constructor({ prefix="", dev=false, penalty_ms=1000, ...args }={}) {
        super();
    ...
    get path() {
        if ( Array.isArray(this.constructor.path) ) {
            return this.constructor.path.map( p => join(this._prefix, p) );
        }
        return join( this._prefix, this.constructor.path );
    }
    ...
    init(args) {} // eslint-disable-line no-unused-vars
